...
final class PhoneController extends AbstractController
{
    ...
    #[OA\RequestBody(required: true, content: new OA\JsonContent(example: ["number" => "+33612345678", "status" => 1]))]
    /**
     * Create a new phone
     */
    public function add(Request $request, SerializerInterface $serializerInterface, EntityManagerInterface $entityManagerInterface, UrlGeneratorInterface $urlGenerator, TagAwareCacheInterface $cache, ValidatorInterface $validator): JsonResponse
    {
        ...
        if (!isset($data['status'])) {
            return new JsonResponse(["error" => "Le champ 'status' est requis"], JsonResponse::HTTP_BAD_REQUEST);
        }

        $status = $entityManagerInterface->getRepository(Status::class)->find($data['status']);
        if (!$status) {
            return new JsonResponse(["error" => "Le status fourni est invalide"], JsonResponse::HTTP_BAD_REQUEST);
        }
        ...
        $response = $urlGenerator->generate("phone_get", ["id" => $phone->getId()], UrlGeneratorInterface::ABSOLUTE_URL);
